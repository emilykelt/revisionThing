...
# ---- AnkiConnect (push straight to Anki desktop if it's running) ----

ANKI_CONNECT_URL = os.environ.get('ANKI_CONNECT_URL', 'http://127.0.0.1:8765')
ANKI_DECK_NAME = 'study buddy recommended cards'
ANKI_MODEL_NAME = 'Cambridge IB Revision'
_anki_setup_done = False


def _ankiconnect(action, **params):
    """POST to AnkiConnect. Returns the result, or None if Anki isn't reachable."""
...
def _ensure_anki_deck_and_model():
    """Create deck + note type once per process. Returns False if Anki isn't running."""
    global _anki_setup_done
    if _anki_setup_done:
        return True
    if _ankiconnect('version') is None:
        return False
    try:
        _ankiconnect('createDeck', deck=ANKI_DECK_NAME)
        models = _ankiconnect('modelNames') or []
        if ANKI_MODEL_NAME not in models:
            _ankiconnect(
                'createModel',
                modelName=ANKI_MODEL_NAME,
                inOrderFields=['Front', 'Back'],
                css='.card{font-family:Arial;font-size:16px;white-space:pre-wrap}',
                cardTemplates=[{
                    'Name': 'Card',
                    'Front': '{{Front}}',
        ...
        _anki_setup_done = True
        return True
    except Exception as e:
        print(f'[anki] setup failed: {e}')
        return False


def _push_card_to_anki(card):
    """Add a single card to Anki. Returns True on success."""
    try:
        result = _ankiconnect('addNote', note={
            'deckName': ANKI_DECK_NAME,
            'modelName': ANKI_MODEL_NAME,
            'fields': {'Front': card['front'], 'Back': card['back']},
            'tags': [
                'ai-generated',
                card['topic_id'].replace('-', '_'),
        ...
        print(f'[anki] push failed for "{card["front"][:60]}": {e}')
        return False


def _queue_flashcards(question, model_solution, topic_name, course_name, topic_id, course_id):
    """Generate flashcards in a background thread. Push to Anki if running, else queue to bank."""
    import threading
    def _run():
        try:
            ...
            anki_ready = _ensure_anki_deck_and_model()
            pushed = 0
            unsynced = []
            for c in cards:
                card = {
                ...
                if anki_ready and _push_card_to_anki(card):
                    pushed += 1
                    continue
                unsynced.append(card)
            if unsynced:
                bank = _load_anki_bank()
                for c in unsynced:
                    c['id'] = f'{topic_id}-{len(bank)}'
                    bank.append(c)
                _save_anki_bank(bank)
            if pushed:
                print(f'[anki] pushed {pushed} card(s) to Anki ({len(unsynced)} fell back to bank)')
        except Exception as e:
...
@app.route('/api/anki/sync-bank', methods=['POST'])
def api_anki_sync_bank():
    """Push every banked card to Anki via AnkiConnect. Synced cards leave the bank;
    cards that AnkiConnect rejects (e.g. duplicates) stay so the user can inspect."""
    if not _ensure_anki_deck_and_model():
        return jsonify({
            'error': 'Anki is not running, or AnkiConnect add-on (#2055492159) is not installed.',
            'connect_url': ANKI_CONNECT_URL,
        }), 503

    bank = _load_anki_bank()
    if not bank:
        return jsonify({'pushed': 0, 'failed': 0, 'remaining': 0})

    still_pending = []
    pushed = 0
    for card in bank:
        if _push_card_to_anki(card):
            pushed += 1
        else:
            still_pending.append(card)
    _save_anki_bank(still_pending)
    return jsonify({
        'pushed': pushed,
...
def _pdf_to_page_images(pdf_bytes, max_long_edge=1800, max_pages=20):
    """Render the first `max_pages` of `pdf_bytes` to PNGs via pdftoppm,
    scaled so each page's longer edge is `max_long_edge` pixels. Returns
    (images, error) where images is a list of {'media_type', 'data'}
    base64-encoded PNGs suitable for Claude image input.

    Pixel-scaling (not DPI) is intentional: phone-scanned PDFs can have
    page dimensions that are physically huge (e.g. 25") and a fixed DPI
    blows up render time to minutes. Scaling to a target pixel size keeps
    cost predictable regardless of source page size."""
    import subprocess
    import tempfile
    import base64
    import glob
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        try:
            ...
            return [], 'Could not render PDF to page images. Is it a valid PDF?'
        except subprocess.TimeoutExpired:
            print('[handwritten] pdftoppm timed out')
            return [], (f'PDF rendering timed out after 180s. Try a smaller file '
                        f'(under {max_pages} pages).')
...
def api_submit_handwritten():
    ...
    page_images, render_err = _pdf_to_page_images(pdf_bytes)
    if render_err:
        return jsonify({'error': render_err}), 400
...
if __name__ == '__main__':
    import os
    # Default to 5847: avoids 5000 (AirPlay) and 5001 (commonly squatted).
